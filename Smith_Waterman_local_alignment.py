...
def smith_waterman_local_alignment(string_one, string_two, match, missMatch, gap):
    ...
    while locationsLen :
        ...
        # Minimizing Cost
        one, tow = [len(solution_string_one), len(solution_string_two)]
        ...
        # Adding leftout parts
        solution_string_one = solution_string_one + "|"
        solution_string_two = solution_string_two + "|"
        
        solution_string_one = solution_string_one + string_one[maxloc[1]:len(string_one)]
        solution_string_two = solution_string_two + string_two[maxloc[0]:len(string_two)]
        
        solutionLenOne = len(solution_string_one)
        solutionLenTwo = len(solution_string_two)
        
        leftour = abs(solutionLenOne - solutionLenTwo)
        
        if solutionLenOne > solutionLenTwo:
            solution_string_two = solution_string_two + ('_' * leftour)
        else:
            solution_string_one = solution_string_one + ('_' * leftour)
        ...
        result.append((solution_string_one, solution_string_two))
